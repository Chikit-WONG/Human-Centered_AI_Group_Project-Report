...
The five seed-level subject-dependent macro means range from $85.60\%$ to $87.35\%$ Top-1 and from $98.20\%$ to $98.55\%$ Top-5, yielding $86.66\pm0.69\%$ and $98.38\pm0.14\%$. Our method is numerically strongest among the displayed intra-subject rows, but protocol differences preclude an unqualified state-of-the-art claim. In strict LOSO, the macro mean drops to $20.40\pm6.91\%$ Top-1 and $49.30\pm10.47\%$ Top-5 across held-out subjects. Among the displayed inter-subject rows, these means exceed NICE, ATM-S, UBP, and NeuroBridge, but trail Shallow Alignment by 2.0 Top-1 and 1.5 Top-5 points; we therefore describe our LOSO result as competitive rather than state of the art. This ordering is consistent with the scope of our design: asymmetric co-adaptation has no explicit subject-invariance objective. MVNN reduces covariance shift, but the flattened MLP neither models electrode topology nor suppresses subject identity, and image-side LoRA observes only the nine source participants. Residual variation in response scale, latency, cap placement, and spatial covariance can therefore remain. Figure~\ref{fig:loso} shows that all folds remain above chance but vary strongly, confirming that subject shift is still a central limitation.
...
The strict LOSO result tests a capability distinct from the paired TTUR ablation: A/B/C establishes that asymmetric co-adaptation helps under a matched subject distribution, not that it guarantees state-of-the-art transfer to an unseen participant. Performance above chance and better macro means than four displayed baselines demonstrate meaningful transfer, while the remaining gap to Shallow Alignment identifies subject invariance as unresolved. Five seeds quantify optimization variability over the same stimuli, not new participants, and literature rows also differ in encoders, preprocessing, and checkpoint rules. The subject-08 matching suite is diagnostic rather than population-level evidence, and batch matching can inflate apparent performance when a gallery is known to be a permutation. We therefore separate retrieval from assignment and test broken assumptions.
